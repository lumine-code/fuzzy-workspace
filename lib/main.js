...
module.exports = {
  ...
  activate(state) {
    ...
    this.selectList = lumine.workspace.buildSelectList({
      ...
      actionsFilter: ({ name }) =>
        name !== "fuzzy-workspace:remove-from-recent" ||
    ...
    this.disposables = new CompositeDisposable(
      ...
      lumine.commands.add(this.selectList.element, {
        ...
        "fuzzy-workspace:remove-from-recent": {
          description: "Drop the selected item from the recent section, leaving the list open.",
  ...
  isRecent(item) {
    return Boolean(item?.uri) && this.recentlyUsed.includes(item.uri);
  },
  ...
  // Forgetting one item is something a user does to several in a row, so the
  // list stays open and the row keeps its selection — it only moves out of
  // the section, down to where its own title puts it.
  removeFromRecent() {
    const item = this.selectList.getSelectedItem();
    if (!this.isRecent(item)) return;
    const uri = item.uri;
    this.recentlyUsed.splice(this.recentlyUsed.indexOf(uri), 1);
    this.refreshRecent();
    // The rows are rebuilt from the live workspace, so the row to keep
    // selected is found again by URI rather than by the object the action
    // started from.
    const row = this.selectList.items?.find((each) => each.uri === uri);
  ...
  performAction(mode) {
    ...
    // Recency is what the user reached for, not only what they focused: every
    // action below acts on this entry, so record it once here rather than in
    // each branch, where a new action would forget to. Closing counts too —
    // the item comes back the moment it is reopened.
